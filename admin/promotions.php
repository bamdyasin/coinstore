...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'approve') {
        $pdo->prepare("UPDATE promotions SET status = 'approved' WHERE id = ?")->execute([$id]);
    } elseif ($action == 'complete') {
        $pdo->prepare("UPDATE promotions SET status = 'completed' WHERE id = ?")->execute([$id]);
    } elseif ($action == 'reject') {
        $pdo->prepare("UPDATE promotions SET status = 'rejected' WHERE id = ?")->execute([$id]);
    }
    // Redirect back to the same tab
    header("Location: promotions.php?tab=" . $current_tab);
    exit();
}

// Fetch promotions for the current tab
$stmt = $pdo->prepare("SELECT * FROM promotions WHERE status = ? ORDER BY created_at DESC");
...
foreach ($allowed_tabs as $tab) {
    $counts[$tab] = $pdo->query("SELECT COUNT(*) FROM promotions WHERE status = '$tab'")->fetchColumn();
}
...
if ($current_tab == 'pending') {
    ...
    foreach ($pending_promos as $promo) {
        ...
        if ($match) {
            ...
            $pdo->prepare("UPDATE promotions SET status = 'approved' WHERE id = ?")->execute([$promo['id']]);
            
            // 2. Mark the TrxID as 'used' so it can't be reused
            $pdo->prepare("UPDATE trxids SET status = 'used' WHERE id = ?")->execute([$match['id']]);
            
            // Note: We don't redirect here to allow the loop to finish, 
            // but we'll refresh the page once after all checks if any update happened.
            $any_updated = true;
        }
    }
    
    if (isset($any_updated)) {
        header("Location: promotions.php?tab=pending&auto_approved=1");
        exit();
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .info-label { color: #777; font-weight: 500; width: 90px; display: inline-block; }
...
<body style="background: #f0f2f5;">
    ...
    <div class="admin-container">
        ...
        <?php foreach ($promotions as $p): ?>
            <div class="request-card <?php echo $p['status']; ?>">
                <div class="card-header">
                    <span class="request-id">ID: #<?php echo $p['id']; ?></span>
                    <span style="font-size: 0.7rem; color: #aaa;"><?php echo date('d M, Y | h:i A', strtotime($p['created_at'])); ?></span>
                </div>

                <div class="info-row">
                    <span class="info-label">Video:</span>
                    <a href="<?php echo htmlspecialchars($p['video_link']); ?>" target="_blank" style="color: #3498db; word-break: break-all; font-size: 0.85rem;">Open Link 🔗</a>
                </div>
                <div class="info-row">
                    <span class="info-label">Type:</span>
                    <span class="info-value"><?php echo htmlspecialchars($p['category']); ?></span>
                </div>
                <div class="info-row">
                    <span class="info-label">Budget:</span>
                    <span class="info-value">৳<?php echo number_format($p['budget'], 2); ?></span>
                </div>
                <div class="info-row">
                    <span class="info-label">WhatsApp:</span>
                    <span class="info-value"><?php echo htmlspecialchars($p['whatsapp']); ?></span>
                </div>
                <div class="info-row">
                    <span class="info-label">Payment:</span>
                    <span class="info-value"><?php echo htmlspecialchars($p['payment_option']); ?></span>
                </div>
